...
class Title(db.EmbeddedDocument):
  language: dicts.Language = db.ReferenceField(document_type='dicts.Language', primary_key=True)
  title: str = db.StringField(max_length=255, required=True)


class Poster(db.EmbeddedDocument):
  language: dicts.Language = db.ReferenceField(document_type='dicts.Language', primary_key=True)
  paths: list[str] = db.ListField(field=db.StringField(max_length=255, required=True, unique=True), default=list)
...
class CurrencyAmount(db.EmbeddedDocument):
  currency: dicts.Currency = db.ReferenceField(document_type='dicts.Currency', primary_key=True)

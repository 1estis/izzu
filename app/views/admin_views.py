...
from ..node_modules_fixer import fix_relative_references
...
def dictionaries():
    return render_template('admin/dictionaries.html', active_page='dictionaries', dictionaries=[
        ...
        for _dict in [Language, ContentType]
    ])
...
# @login_required
# @roles_required('admin')
# def add_to_dictionary(d_name):
#     _dict = globals()[d_name]
#     _dict(**request.form).save()
#     return redirect(url_for('admin.dictionaries'))

# @admin_blueprint.route('/admin/dictionary/<string:d_name>/edit/<string:o_id>', methods=['POST'])
# @login_required
# @roles_required('admin')
# def dictionary_object(d_name, o_id):
#     _dict = globals()[d_name]
#     _dict.objects(id=o_id).update(**request.form)
#     return redirect(url_for('admin.dictionaries'))
